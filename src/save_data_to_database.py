        ...
        for dict_data in data:
            ...
            cur.execute(
                ...
                (employer_data["id"], employer_data["name"], employer_data["site_url"],
                 employer_data["city"], employer_data["open_vacancies"])
            )
            ...
            for vac in vacancies_data:
                ...
                cur.execute(
                    """
                    INSERT INTO vacancies (vacancies_id, name, city, salary_from, salary_to, currency, employers_id,
                    vacancies_url, published_at)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
                    """,
                    (vac["id"], vac["name"], vac["area"]["name"], salary_from, salary_to,
                     currency, vac["employer"]["id"], vac["employer"]["vacancies_url"], vac["published_at"])
                )
    conn.commit()
    conn.close()
